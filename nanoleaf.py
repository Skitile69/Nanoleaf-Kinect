...
Description:
    ...
    Each row in the display has a dedicated controller except 1st two row and last two row. Total there are 12 row and 10 controller.
    
    This code is extened from the Demo script of basic OpenAPI functionality for interacting with Nanoleaf Light Panels.
    Which Included:
...
def  UofCGradient(positionD):
    ...
    parameter description:
        ...
        In the funtion only the panelIDs are being used.
    '''
...
def z_of_zetta(positionD):
    ...
    #The flag_list is used in this function but it is used when the aphabet is displayed in a changing background
    flag_list=[False,False,True,True,True,True,True,True,False,False]
    T=5
    counter=0 # for row 7 and 8(in otherword controller 5 and 6) we need to light up 12th and 13th, and 10th and 11th respectively. 
...
def constant_color(positionData1,positionData2,positionData3,positionData4,positionData5,positionData6,positionData7,positionData8,positionData9,positionData10):
    T=5
...
hex_count=0
hex_flag=[True,True,True,True,True,True,True,True,True,True]# hex_flag indicates the controllers which have all its connected panels change color 
...
def hexagon(positionD,R1,G1,B1):
    '''
    This function covers the whole display with hexagons that are colordefined in the parameter, starting from the most outer one.
    ...
    for i,panelData in enumerate(positionD):
        if hex_count==0:
            if i==0:
                for j,panel in enumerate(panelData):
                    if (j>13 and j<27) or j<2 or j==13 or j==27:
                        R=R1
                        G=G1
                        B=B1
                        frame = {'panelId': panel['panelId'], 'R': R, 'G': G, 'B': B, 'T': T}
        ...
        if j==4:
            for i,panel in enumerate(panelData):
                if zetta_counter==0:
                    ...
                    if (i>15 and i<18) or (i>18 and i<21):
                        R=255
                        G=103
                        B=31
                    else:
...
if __name__ == "__main__":
    ...
    #authentication keys are also required to establish connection. Holding the power button of the controller for around 30s
    #will generate a new authication key. And if you have created a new authentication key, use getAuth(ip) function to get new key                             
    ...
    auth6="eRTQpXfNZh4KLdvnJXsdQMsSzspSerPu"
    auth7="ALf4TvPrNiK4hxEVdjtbc5Gl0X2l0NCl"
    auth8="eRTQpXfNZh4KLdvnJXsdQMsSzspSerPu"
    auth9="eRTQpXfNZh4KLdvnJXsdQMsSzspSerPu"
    ...
    for i,panel in enumerate(positionData5):
        if i==4 or i==5:
            ...
            G=10
            B=114
        elif (i>15 and i<18) or (i>18 and i<21):
            R=255
            G=103
            B=31
        else:
    ...
    ''''
    # from here I am sending commands to the controllers
    '''    
    while 1:
